...
export async function publishWork(workId: string, title: string, content: string, transferToOss: boolean) {
  ...
  const fingerprint = createHash('sha256').update(JSON.stringify({ title, content, images, transferToOss })).digest('hex')
  const [existing] = await db.select().from(publications).where(and(eq(publications.workId, workId), eq(publications.fingerprint, fingerprint)))
  if (existing) return { ...existing, cached: true }
  const response = await fetch(env.XHS_PUBLISH_API_URL, { method: 'POST', headers: { Authorization: `Bearer ${env.AICONDUCTOR_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'normal', title, content, images, transfer_to_oss: transferToOss }) })
  ...
  const record = { id: result.data.id ?? randomUUID(), workId, fingerprint, h5Url: result.data.url, qrCode: result.data.qrcode, transferToOss, serviceFee: String(result.billing?.service_fee ?? ''), currency: result.billing?.currency ?? 'CNY', transactionId: result.billing?.transaction_id ?? null, createdAt: new Date() }
