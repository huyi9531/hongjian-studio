...
export const Route = createFileRoute('/api/retry-failed')({
  server: { handlers: { POST: async ({ request }) => {
    ...
    const stream = new ReadableStream({ start(controller) {
      ...
      void retryFailedWorkImages(parsed.data.workId, parsed.data.model, parsed.data.size, emit, parsed.data.promptMode).catch(cause => emit({ event: 'error', data: { index: -1, message: cause instanceof Error ? cause.message : String(cause), retryable: false } })).finally(() => controller.close())
    } })
