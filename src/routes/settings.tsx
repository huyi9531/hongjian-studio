...
import { imagePromptModes } from '@/lib/studio-preferences'
  ...
  return <AppShell><section className="mx-auto max-w-5xl px-4 py-8 sm:px-8 sm:py-10 lg:px-10">
    ...
    <div className="mt-7 grid gap-0 overflow-hidden rounded-2xl bg-card">
      ...
      <section className="border-t border-hairline p-5 sm:p-6"><div className="mb-4"><h2 className="text-base font-medium">图片提示词</h2><p className="mt-1 text-sm text-muted-foreground">选择生成图片时发送给模型的提示词详细程度。</p></div><div className="grid gap-3 sm:grid-cols-2">{[
        { value: imagePromptModes.short, label: '短版（默认）', description: '只保留页面内容、类型、合规与 3:4 排版要求，指令更直接。' },
        { value: imagePromptModes.long, label: '长版', description: '附带原始需求、完整大纲、页面类型细则与封面风格继承要求。' },
      ].map(mode => <button key={mode.value} type="button" aria-pressed={preferences.imagePromptMode === mode.value} onClick={() => setPreferences(current => ({ ...current, imagePromptMode: mode.value }))} className={cn('flex min-h-24 items-start justify-between rounded-xl bg-muted p-5 text-left transition-[background-color,transform] hover:bg-accent active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring-soft', preferences.imagePromptMode === mode.value && 'bg-primary-soft')}><span><span className="block text-sm font-medium">{mode.label}</span><span className="mt-2 block text-xs leading-5 text-muted-foreground">{mode.description}</span></span>{preferences.imagePromptMode === mode.value && <span className="grid size-7 shrink-0 place-items-center rounded-full bg-primary text-white"><Check className="size-4" aria-hidden="true" /></span>}</button>)}</div></section>
